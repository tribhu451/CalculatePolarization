...
void Init::InitTJb(SCGrid &arena_prev, SCGrid &arena_current) {
    if (DATA.Initial_profile == 0) {
       ...
       std::cout << "Ready for initialisation" << std::endl ;

	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// This function is introduced by TRIBHUBAN to read the 3D initial distribution. 						//
        initial_IPGlasma_XY_TRIBHUBAN( arena_prev, arena_current ); // Edited by TRIBHUBAN						//
	                                                            // A corresponding modification can be found in header file also.	//
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void Init::initial_IPGlasma_XY(int ieta, SCGrid &arena_prev,
    ...
    double temp_profile_ux[nx][ny];
    double temp_profile_uy[nx][ny];
    double temp_profile_rhob[nx][ny]; // added by tribhuban

    // read the one slice
    double density, dummy1, dummy2, dummy3;
    double ux, uy, utau,nb; // added by tribhuban
    for (int ix = 0; ix < nx; ix++) {
        for (int iy = 0; iy < ny; iy++) {
            profile >> dummy1 >> dummy2 >> dummy3
                    >> density >> utau >> ux >> uy
                    >> dummy  >> nb  >> dummy  >> dummy; // added by tribhuban
            temp_profile_ed[ix][iy] = density;
            temp_profile_ux[ix][iy] = ux;
            temp_profile_uy[ix][iy] = uy;
            temp_profile_rhob[ix][iy] = nb; // added by tribhuban
    ...
    for (int ix = 0; ix < nx; ix++) {
        for (int iy = 0; iy< ny; iy++) {
            ...
            rhob = (temp_profile_rhob[ix][iy]*eta_envelop_ed);  // added by tribhuban
  ...
  // Tribhuban modified the "initial_IPGlasma_XY"
  // to implement 3D initial distribution to the 
  // code

void Init::initial_IPGlasma_XY_TRIBHUBAN(  SCGrid &arena_prev,
                               SCGrid &arena_current) {


    std::cout << "[Info] [TRIBHUBAN] Reading a 3+1D initial distribution file." << std::endl ; 
    ifstream profile(DATA.initName.c_str());
    ...
    double density, dummy1, dummy2, dummy3, dummyxx ;
    double ux, uy, utau, ueta, nb, yL ; 
    for (int ieta = 0; ieta < neta; ieta++) {
      for (int ix = 0; ix < nx; ix++) {
        for (int iy = 0; iy < ny; iy++) {
	  profile >> dummy1 >> dummy2 >> dummy3
		  >> density >> yL >> dummyxx >> dummyxx
		  >> dummyxx  >> nb  >> dummy  >> dummy;
           
	  double pressure = eos.get_pressure(density*DATA.sFactor, nb );
          double xx = density * DATA.sFactor * sinh(yL) / ( density * DATA.sFactor * cosh(yL) + pressure ) ;
	   
	   if( ( xx * xx / ( 1 - xx * xx ) ) < 0.0 ){
             std::cout << "ueta = nan" << std::endl ; 
	     exit(1) ; 
	   }  
	   ...
	   if( fabs(yL) < 0.0001 ) 
	     ...
	     ueta = ( yL / fabs(yL) ) *  sqrt( xx * xx / ( 1 - xx * xx ) );
	   ux   = 0. ;  
	   uy   = 0. ;  
	  ...
	  if ( ( ieta ==0 && ix == 0 ) && iy == 0) {
	    ...
	    if (entropy_flag == 0) {
	      ...
	      epsilon = eos.get_s2e(local_sd, nb);
            }
