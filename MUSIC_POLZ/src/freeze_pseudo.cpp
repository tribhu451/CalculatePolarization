...
void Freeze::ComputeParticleSpectrum_pseudo_improved(InitData *DATA,
    ...
    //edit by Koustav
    //--------------------------------------
    // set some parameters to set the momentum grid
    ...
    //edit by Koustav --------------------------------------
    // set particle properties (mass, deg and baryon number for the distribution function)
    ...
    // open files to write the particle spectrum
    FILE *d_file;
    ...
    fprintf(d_file, "%d %e %d %e %e %d %d \n",
            ...
            iptmax, iphimax);
    //edit by Koustav --------------------------------------
    //sampling the grid points, equidistant in phi but sampled quadratically in pt
    // caching 
    ...
    for (ieta = 0; ieta < ietamax; ieta++) {
        ...
        double** temp_sum = new double* [iptmax]; //pt-phi grid
        ...
        #pragma omp parallel
        {
            ...
            #pragma omp for
            //loop to access the Surface elemnts on the hypersurface 
        ...
        for (int i = 1; i < particleMax_copy; i++) {
            ...
            for(int part = 1; part < i; part++) {
                ...
                if (mass_diff < mass_tol && mu_diff < mu_tol
                    ...
                    //momentum grid information declaration, passed from the initial input parameters
