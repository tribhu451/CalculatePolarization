...
template<class T>   //making the grid template
class GridT {
 private:
    std::vector<T> grid;

    int Nx   = 0;           //grid parameters
    int Ny   = 0;
    int Neta = 0;
  
    T& get(int x, int y, int eta) {                     //function to 'get' a particular cell from the grid (return by reference)
 ...
 public:
    ...
    T& getHalo(int x, int y, int eta){
        //check the bounds for the cell indices
...
//picks out (-2, -1, 0, 1, 2) neighbourhood cells for any given direction 


//(check use in u_derivative.cpp)
