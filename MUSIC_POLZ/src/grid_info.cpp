    ...
    for (int ieta = 0; ieta < arena.nEta(); ieta += n_skip_eta) {
        ...
        for (int iy = 0; iy < arena.nY(); iy += n_skip_y) {
            for (int ix = 0; ix < arena.nX(); ix += n_skip_x) {
                ...
                double x = ((static_cast<double>(ix))*(DATA.delta_x)
                    - (DATA.x_size)/2.0);
                double y = ((static_cast<double>(iy))*(DATA.delta_y)
                    - (DATA.y_size)/2.0);
                ...
                // exclude the actual coordinates (tau, x, y, eta) from the output to save space:
                if (DATA.outputBinaryEvolution == 0) {
                    
                    //edit : do not exclude the actual coordinates (tau, x, y, eta) from the output:
                    fprintf(out_file_xyeta, "%e %e %e %e %e %e %e %e %e %e\n", 
                            tau, x, y, eta, e_local*hbarc, T_local*hbarc, muB_local*hbarc, vx, vy, vz);
