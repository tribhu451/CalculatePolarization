...
void Freeze::ReadFreezeOutSurface(InitData *DATA) {
    ...
    if (surface_in_binary) {
        ...
        if (!surfdat.is_open())
        {
            music_message << "Error in reading the initial profile: " << surfdat_stream.str()
                          << "not found!!!";
            music_message.flush("error");  
            exit(1);
        }
    ...
    while (i < NCells) {
        ...
        if (surface_in_binary) {
            ...
            //passing the vorticity vectors
            double dummy_hyps;
            surfdat >> dummy_hyps >> dummy_hyps >> dummy_hyps >> dummy_hyps
                    >> dummy_hyps >> dummy_hyps >> dummy_hyps >> dummy_hyps 
                    >> dummy_hyps >> dummy_hyps >> dummy_hyps >> dummy_hyps
                    >> dummy_hyps >> dummy_hyps >> dummy_hyps >> dummy_hyps;
        }
