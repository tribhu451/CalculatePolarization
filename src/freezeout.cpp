        ...
        for (int icell = 0; icell < N_hypercells; icell++) {
                ...
                if (DATA->turn_on_bulk == 1 && DATA->include_deltaf_bulk == 1) {
                    ...
                    {       
                        ...
                        for (int mu = 0; mu < 4; mu++) 
                            for (int nu = 0; nu < 4; nu++) d_mu_betanu[mu][nu] = 0.0;

                        //from the hypersurface, one obtains d_\mu \beta^{\nu}
                        for (int mu = 0; mu < 4; mu++) 
                            for (int nu = 0; nu < 4; nu++) d_mu_betanu[mu][nu] = hypersurface_cells[icell].d_mu_betanu[mu][nu];
...
void freezeout :: calc_polarization(){
    ...
    double eta_cut = 4.0;                                                               
    double a_pt, b_phi, c_y, tempc_y;
    double spin = 0.5;                                                                  //spin-of the particle is hard coded for time-being
